...
- Loads the equilibrated PDB and OpenMM state file (.rst XML).
- Preserves periodic box vectors.
- Saves DCD, PDB, state (.rst), and checkpoint at each stride.
- Recommended usage: nohup python 09_production_md.py &
"""
...
# -----------------------------
Stride_Time_ns = 5         # length of each stride [ns]
Number_of_strides = 11     # number of strides
Integration_timestep_fs = 2  # fs
Temperature_K = 300        # K
Pressure_bar = 1           # bar
Frames_per_stride = 100    # frames saved per stride
Checkpoint_ps = 100        # checkpoint frequency [ps]
...
# Save exactly Frames_per_stride frames per stride
nsavcrd = nsteps // Frames_per_stride
nchk = int(Checkpoint_ps*unit.picoseconds/dt)
...
for stride in range(1, nstride+1):
    ...
    # Load state from previous stride (or equilibration for stride 1)
    if stride == 1:
    ...
    simulation.reporters.append(StateDataReporter(
        stdout, nsavcrd,
        step=True, speed=True, progress=True, remainingTime=True,
    ...
    print(f"> Running {nsteps} steps (Stride #{stride})...")
    ...
    simulation.reporters.clear()  # clear reporters for next stride
